do
    ...
    local function safeCall(func, ...)
        ...
        if not success then
            warn("SpecialUI Error: " .. tostring(result))
            return nil
        end
        return result
    end
    ...
    function SpecialUI:DraggingEnabled(frame, parent, instanceId)
        ...
        local function update(input)
            ...
            dragTween = safeCall(tween.Create, tween, parent, TweenInfo.new(0.15, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), { Position = targetPos })
        ...
        local conn1 = frame.InputBegan:Connect(function(i)
            ...
            if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
                ...
                AddConnection(conn)
                table.insert(DraggingConnections[instanceId], conn)
            end
        end)
        AddConnection(conn1)
    ...
    function SpecialUI.DestroyUI()
        if ThemeEvent then
            safeCall(ThemeEvent.Destroy, ThemeEvent)
            ThemeEvent = nil
        end
        ...
        for i = #ActiveObjects, 1, -1 do
            ...
            if obj and obj.Parent then
                safeCall(obj.Destroy, obj)
            end
            ActiveObjects[i] = nil
        end
        if MainInstance and MainInstance.Parent then
            safeCall(MainInstance.Destroy, MainInstance)
            MainInstance = nil
        end
        local gui = game.CoreGui:FindFirstChild(LibName)
        if gui then
    ...
    function SpecialUI.CreateLib(kavName, themeName)
        ...
        if MainCorner then
            ...
            MainCorner.Parent = MainInstance
        end
