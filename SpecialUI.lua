...
local ThemeStyles = {
    ...
    Mint = { SchemeColor = Color3.fromRGB(200, 255, 160), Background = Color3.fromRGB(0, 0, 0), Header = Color3.fromRGB(5, 5, 5), TextColor = Color3.fromRGB(255, 255, 255), ElementColor = Color3.fromRGB(15, 15, 15) },
    Terminal = { SchemeColor = Color3.fromRGB(0, 170, 255), Background = Color3.fromRGB(0, 0, 0), Header = Color3.fromRGB(10, 10, 10), TextColor = Color3.fromRGB(0, 255, 170), ElementColor = Color3.fromRGB(15, 15, 15) }
...
function SpecialUI:RegisterElement(name, factory)
    CustomElements[name] = factory
end

function SpecialUI:RegisterExtension(name, value)
    self[name] = value
end
...
function SpecialUI:Notify(text, duration)
    duration = duration or 3
    local textColor = themeList.SchemeColor
    local backgroundColor = themeList.Background
...
function SpecialUI.CreateLib(kavName, themeName)
    ...
    if close then
        ...
        local closeConn = close.MouseButton1Click:Connect(function() SpecialUI:Destroy() end)
    ...
    MobileToggleGui.Name = "MobileToggle" .. LibName
    MobileToggleGui.Parent = plr:WaitForChild("PlayerGui")
    MobileToggleGui.ResetOnSpawn = false
    AddObject(MobileToggleGui)

    local Toggle = Instance.new("TextButton")
    Toggle.Name = kavName .. "Toggle"
    Toggle.Parent = MobileToggleGui
    Toggle.BackgroundColor3 = themeList.Background
    Toggle.Position = UDim2.new(0, 10, 0.5, -20)
    Toggle.Size = UDim2.new(0, 95, 0, 38)
    ...
    Stroke.Color = themeList.SchemeColor
    Stroke.Thickness = 1.5
    Stroke.Transparency = 0.4
    Stroke.Parent = Toggle

    local hoverEnterConn = Toggle.MouseEnter:Connect(function()
        Toggle.BackgroundColor3 = Color3.fromRGB(
            math.clamp(themeList.Background.R * 255 + 25, 0, 255),
            math.clamp(themeList.Background.G * 255 + 20, 0, 255),
            math.clamp(themeList.Background.B * 255 + 35, 0, 255)
        )
        Stroke.Transparency = 0.1
    end)
    AddConnection(hoverEnterConn)

    local hoverLeaveConn = Toggle.MouseLeave:Connect(function()
        Toggle.BackgroundColor3 = themeList.Background
        Stroke.Transparency = 0.4
    end)
    AddConnection(hoverLeaveConn)

    local toggleClickConn = Toggle.MouseButton1Click:Connect(function()
        SpecialUI:ToggleUI()
    end)
    AddConnection(toggleClickConn)

    local function UpdateMobileToggleTheme()
        if Toggle and Toggle.Parent then
            Toggle.BackgroundColor3 = themeList.Background
            Toggle.TextColor3 = themeList.TextColor
            if Stroke then Stroke.Color = themeList.SchemeColor end
        end
    end

    local mobileThemeConn = ThemeEvent.Event:Connect(UpdateMobileToggleTheme)
    AddConnection(mobileThemeConn)
        ...
        function Sections:NewSection(secName, hidden)
            ...
            setmetatable(Elements, {
                __index = function(tbl, key)
                    if CustomElements[key] then
                        return function(self, ...)
                            local result = CustomElements[key](sectionInners, themeList, ThemeEvent, AddConnection, UpdateSize, updateSectionFrame, ...)
                            updateSectionFrame()
                            UpdateSize()
                            return result
                        end
            ...
            function Elements:NewColorPicker(colText, colInf, defcolor, callback)
                ...
                local function setcolor(tbl)
                    ...
                    cursor.Position = UDim2.new(color[1], -cx, 1 - color[2], -cy)
            ...
            function Elements:NewHomeCard(config)
                ...
                for i, btnData in ipairs(buttons) do
                    if i <= 2 then
                        ...
                        local homeBtnEnter = btn.MouseEnter:Connect(function()
                            tween:Create(btn, TweenInfo.new(0.15), {BackgroundTransparency = 0}):Play()
                        end)
                        AddConnection(homeBtnEnter)

                        local homeBtnLeave = btn.MouseLeave:Connect(function()
                            tween:Create(btn, TweenInfo.new(0.15), {BackgroundTransparency = 0.2}):Play()
                        end)
                        AddConnection(homeBtnLeave)

                        local homeBtnClick = btn.MouseButton1Click:Connect(function()
            ...
            function Elements:NewConsolePlayer(captureLogService)
                ...
                PrintToConsole("SpecialUI Cheesecake v" .. VERSION, themeList.SchemeColor)
...
function SpecialUI:Destroy()
    ...
    local mobileGui = game.Players.LocalPlayer:WaitForChild("PlayerGui"):FindFirstChild("MobileToggle" .. LibName)
    if mobileGui then
